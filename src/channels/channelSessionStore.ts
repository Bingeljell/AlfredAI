...
import { readJsonFile, updateJsonFile, withFileLock } from "../utils/fs.js";
...
export class ChannelSessionStore {
  ...
  async getOrCreate(key: string, create: () => Promise<ChannelSessionRecord>): Promise<ChannelSessionRecord> {
    return withFileLock(`${this.filePath}.${encodeURIComponent(key)}.binding`, async () => {
